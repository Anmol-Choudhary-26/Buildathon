...
export default function ListingCard({ listing, token, onRequestCreated }) {
  ...
  return <article className="listing-card">
    {listing.media?.length > 0 && <div className="property-media">{listing.media.map((asset, index) => asset.type === 'video' ? <video key={asset.url} controls preload="metadata"><source src={asset.url} /></video> : <img key={asset.url} src={asset.url} alt={`Property view ${index + 1}`} loading="lazy" />)}</div>}
